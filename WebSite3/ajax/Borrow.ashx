<%@ WebHandler Language="C#" Class="Borrow" %>
using System;
using System.Web;
...
public class Borrow : IHttpHandler {
    public void ProcessRequest (HttpContext context) {
        ...
        if (comm.ExecuteScalar() != null)
        {
            ...
            else if (PersonName.ToString() != "")//身份信息不正确
            {
                ...
                if (co.ExecuteScalar().ToString() != Department.ToString())
                ...
                else
                {
                    ...
                    string sqll = "insert into Borrow(device_name,count,experienced_name,credit_Department,uses,proposed_return_date,appointment,Is_Return) values('" + DeviceName.ToString() + "','" + Count.ToString() + "','" + PersonName.ToString() + "'," +
                        "'" + Department.ToString() + "','" + Use.ToString() + "','" + ReturnDate.ToString() + "','否','" + "未归还" + "')";
    ...
    public string Isappoint(HttpContext context)
    {
        ...
        SqlConnection con = new SqlConnection(strConn);
        con.Open();//代开数据库的连接
        string s = "select appointment from Borrow where device_name='" + DeviceName.ToString() + "'";
